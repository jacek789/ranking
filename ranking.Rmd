...
ggplot(data=corel %>% group_by(x) %>% mutate(mean_c = mean(cor)) %>% ungroup()) +
  ...
  geom_segment(aes(x=n_teams*5 / 2, y=0, xend=n_teams*5/2, yend=0.6)) +
  stat_smooth(aes(x=x, y=mean_c), method="loess", span=0.2, color="black") +
  geom_label(aes(x=n_teams * 5 / 2, y=0.3, label="po 5 meczy")) +  # considerably slower than geom_text, ale robi ładne ramki
  geom_segment(aes(x=n_teams*10 / 2, y=0, xend=n_teams*10/2, yend=0.68)) +
  geom_label(aes(x=n_teams * 10 / 2, y=0.7, label="po 10 meczy")) +
  theme(
